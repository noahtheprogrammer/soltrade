    ...
    if (os.path.exists("config.json")):
        with open('config.json', 'r') as openfile:
            try:
                ...
                print(colors.FAIL + timestamp.find_time() + "Soltrade was unable to parse the JSON file. Are you sure config.json is formatted properly?" + colors.ENDC)
                return None
    else:
        print(colors.FAIL + timestamp.find_time() + "Soltrade was unable to detect the JSON file. Are you sure config.json has not been renamed or removed?" + colors.ENDC)
        return False
